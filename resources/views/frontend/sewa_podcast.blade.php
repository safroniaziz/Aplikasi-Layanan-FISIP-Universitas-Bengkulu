...
@section('sewa_podcast','active-menu')
                                ...
                                <div class="-mx-1 -mb-1">
                                    ...
                                    <div class="flex flex-wrap border-t border-l">
                                        ...
                                        <template x-for="(date, dateIndex) in no_of_days" :key="dateIndex">
                                            <div style="width: 14.28%; height: 130px" class="  border-r border-b relative " :class="{'bg-yellow-200   ': isToday(date) == true, '': isToday(date) == false }">
                                                <div class="flex px-4   bg-yellow-100  bg-opacity-20">
                                                    <div x-text=" date" class="inline-flex -ml-4  pl-4 pr-8 py-2 items-center justify-center   text-center leading-none  rounded-br-full transition ease-in-out duration-100" :class="{'bg-yellow-600 text-white': isToday(date) == true, ' text-gray-700  ': isToday(date) == false }"></div>
                                                    <div @click="showEventModal(date)" class="float-right px-2 mt-1 hover:bg-red-600   hover:scale-95 active:scale-90 duration-300 transform cursor-pointer  absolute right-3 rounded-md bg-red-500 text-white">+</div>
                                                ...
                                                <div style="height: 80px;" class="overflow-y-auto mt-1 px-4 ">
                                                    ...
                                                    <template x-for="event in events.filter(e => new Date(e.event_date).toDateString() === new Date(year, month, date).toDateString())">
                                                        <div x-init="currentDate2 = new Date().toLocaleDateString()" class="px-2 py-1 rounded-lg mt-1 overflow-hidden border" :class="{
                                                            ...
                                                            <p x-html="event.event_title" class="text-sm truncate leading-tight  "> </p>
                                                        </div>
                                                    </template>
                                                </div>
                                            </div>
                ...
                <form action="{{ route('sewaruang.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2   gap-x-6 gap-y-2 my-3">
                        ...
                        <div class=" col-span-2  ">
                            <label class=" after:content-['*'] after:text-red-500 font-semibold text-gray-700 dark:text-gray-300 after:ml-2 text-sm pb-2">NPM</label>
                            <input type="text" class="   w-full rounded-lg border-2 mt-1 border-[#01052D] dark:border-yellow-500
                                bg-transparent px-3 py-2.5 text-sm font-normal text-white-700 transition-all duration-500
                                placeholder:text-gray-600 dark:placeholder:text-yellow-100 focus:border-white
                                dark:focus:ring-yellow-500 focus:ring-[#01052D]
                                focus:shadow-[-4px_4px_10px_0px_#01052D]
                                dark:focus:shadow-[-4px_4px_10px_0px_#eab308] " placeholder="NPM" name="npm" />
                        </div>
                    ...
                    <div class=" col-span-1 grid  grid-cols-3 gap-3">
                        <div class="col-span-2">
                            ...
                            <input type="date" name="tgl_start" class="   w-full rounded-lg border-2 mt-1 border-[#01052D] dark:border-yellow-500
                                ...
                                dark:focus:ring-yellow-500 focus:ring-[#01052D]
                                focus:shadow-[-4px_4px_10px_0px_#01052D]
                                dark:focus:shadow-[-4px_4px_10px_0px_#eab308]   " x-model="event_date2" placeholder="Tanggal Mulai" />
                            @error('tgl_start')
                    ...
                    <div class="mb-4">
                        ...
                        <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded-lg w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="text" x-model="event_title" name="event_title">
                    </div>
                    <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded-lg w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500 hidden" type="text" x-model="event_date" readonly>
                    <div class="mb-4">
                        <label class="text-gray-800 block mb-1 font-bold text-sm tracking-wide">Tanggal Sewa</label>
                        <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded-lg w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="text" x-model="event_date2" name="tanggal_sewa" readonly>
                    </div>
        ...
        return {
            ...
            events: [
                @foreach($pemesananRuangans as $item) {
                    ...
                    event_title: `{{$item->user->name}}`+'<br>'+`{{ $item->mahasiswa_npm }}`,
                    event_theme: 'blue'
                },
                @endforeach
            ],
