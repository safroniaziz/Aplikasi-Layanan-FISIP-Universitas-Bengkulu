...
<a href="{{ route('permohonanSurat') }}" class="px-2 py-2 mx-2 mt-2 @yield('permohonan_surat', 'text-gray-600') lg:text-white  text-[14px] text-sh transition-colors
...
<div class="flex items-center mt-4 mx-4 lg:mt-0">
    <div x-data="{ isOpen: false }" class="relative inline-block ">
        <button @click="isOpen = !isOpen" type="button" class="flex items-center focus:outline-none" aria-label="toggle profile dropdown">
            <img class="w-8 h-8 rounded-full ring-2 mr-1 ring-gray-300"
                    src="https://www.gravatar.com/avatar/{{ md5(Auth::user()->email) }}?d=https%3A%2F%2Fui-avatars.com%2Fapi%2F{{ str_replace(' ', '+', Auth::user()->name) }}/128"
                    alt="Bordered avatar">
